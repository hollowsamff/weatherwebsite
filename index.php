...
<body>
   ...
   <div class="scrollmenu" style="background-color:#F7F7F7; padding:10px">
	...
	<span id="" style="padding-right:5px; opacity: 0.731232;"></span>
    <span class="thick" >Pressure: </span><span  style="opacity:0.731232;" id="pressure"></span> 
	<span id="pressuresign" style="padding-right:15px; opacity: 0.731232;">hPa</span>
  ...
  <div class=" col-sm-offset-1 col-sm-11" style="background-color:white;padding-bottom:0px;margin-bottom:0px">
    ...
    <div class=" col-sm-offset-1 col-sm-11" style="background-color:white;padding-bottom:50px;margin-bottom:50px">
	 <h3>Explore the weather in the past or future</h3>  
            ...
            <div class="row">
                <div class="col-lg-12">
                    <p style = "color:#fff">
						...
						<a href="termsofservice.php"> Terms of use</a>
...
$("document").ready(function() {
	...
	$(".glass").attr("placeholder", "Loading your location address!!!");
    $(document).attr("title", "Loading your location address!!!");
	...
	//Hack to get the search bar to work on some mobiles and tablets
	...
	map.on('geosearch/showlocation', function(data){	
		...
		if(typeof data["location"]["y"] == "undefined") {//Show message when city not found
	...
	function onLocationFound(e) {
        ...
        $(".glass").attr("placeholder", "Loading your location address");
		$(document).attr("title", "Loading your location address");		
		GetAddresses(e["latitude"],e["longitude"]);//Get address from lon / lat
	}
	...
	function findLocation(){
	 ...
	 function  GetAddresses (lat, lng, callback) {
     
		   var url = "https://nominatim.openstreetmap.org/reverse?format=jsonv2&lat="+parseFloat(lat)+"&lon="+parseFloat(lng)+"";
		   ...
		   $.getJSON(url, function (data) {
			  ...
			  .done(function(data) {
				...
				$(".glass").attr("placeholder", data["display_name"]);
				$(document).attr("title", data["display_name"]);
				
			  })
			  .fail(function() {
				
				alert( "Name of your location not found" );
				$(".glass").attr("placeholder", "Showing your location");
		        $(document).attr("title", "Showing your location");
  ...
  function showLocalWeather(lat, lng) {
	...
	if(uniqueArray.length  == 1){
		...
		for(var i = 0; i < uniqueArray.length; i++ ){//Create bar of weather type per hour for the next 22 hours
			...
			if(uniqueArray[i].indexOf('Mostly Cloudy') != -1){
				
				backGroundColor = "#B6BFCB";
				valueToShow = " Mostly Cloudy";
				
			}
			if (uniqueArray[i].indexOf('Overcast') != -1){
			...
			if (uniqueArray[i].indexOf('Partly Cloudy') != -1){
					
					backGroundColor = "#D5DAE2";
					valueToShow = "Partly Cloudy";
					
			}
			if (uniqueArray[i].indexOf('Clear') != -1){
				
					backGroundColor = "#EEEEF5";
					valueToShow = "Clear";
					
			}
			if(uniqueArray[i].indexOf('Rain') != -1){
				
					backGroundColor = "#4A80C7";
					valueToShow = "Rain";
					
			}
			if (uniqueArray[i].indexOf('Light Rain') != -1){
...
$("#datepicker").datepicker({
    onSelect: function() { 
	  ...
	  window.location.href = "timemachines.php?lat="+$("#locatebutton").attr("lat")+"&lng="+$("#locatebutton").attr("lng")+"&day="+daysBetweenDates+"&cityname="+$(".glass").attr("placeholder");
